...
class RayPPOTrainer:
    ...
    def _validate(self) -> dict[str, Any]:
        ...
        for batch_dict in self.val_dataloader:
            ...
            # # Pop claims and labels from new_batch into anc_batch, and also copy visual anchor keys from gen_batch_output into anc_batch
            # claim_keys = [f"claim_{i}" for i in range(1, 21)]
            # label_keys = [f"label_{i}" for i in range(1, 21)]
            # anchor_keys = ["raw_prompt_ids", "raw_response_ids"]

            # # Pop claims and labels from new_batch
            # anc_batch = test_batch.pop(
            #     non_tensor_batch_keys=claim_keys + label_keys,
            # )
            # anc_batch = anc_batch.repeat(repeat_times=repeat_times, interleave=True)

            # # Pop anchor-related keys from gen_batch_output, but only pop raw_prompt_ids and raw_response_ids; copy multi_modal_data
            # anchor_info = test_output_gen_batch.pop(
            #     non_tensor_batch_keys=anchor_keys,
            # )
            # anc_batch.non_tensor_batch.update(anchor_info.non_tensor_batch)

            # # Copy multi_modal_data instead of popping
            # anc_batch.non_tensor_batch["multi_modal_data"] = deepcopy(test_output_gen_batch.non_tensor_batch["multi_modal_data"])
            # anc_batch.meta_info = deepcopy(test_output_gen_batch.meta_info)
            # # Assign the batch from gen_batch_output for device placement
            # anc_batch.batch = test_output_gen_batch.batch

            # # generate visual anchors and anchor scores
            # anc_batch, pad_size = pad_dataproto_to_divisor(anc_batch, self.actor_rollout_ref_wg.world_size)
            # anc_batch_output = self.actor_rollout_ref_wg.generate_anchors(anc_batch)
            # anc_batch_output = unpad_dataproto(anc_batch_output, pad_size=pad_size * repeat_times)

            # repeat to align with repeated responses in rollout
            test_batch = test_batch.repeat(repeat_times=repeat_times, interleave=True)
            #anc_batch_output.meta_info = test_output_gen_batch.meta_info
            test_batch = test_batch.union(test_output_gen_batch)
            #test_batch = test_batch.union(anc_batch_output)
    ...
    def _make_batch_data(self, metrics: dict[str, Any]) -> DataProto:
        ...
        while True:
            ...
            # # Pop claims and labels from new_batch
            # anc_batch = new_batch.pop(
            #     non_tensor_batch_keys=claim_keys + label_keys,
            # )
            # anc_batch = anc_batch.repeat(repeat_times=self.config.worker.rollout.n, interleave=True)

            # # Pop anchor-related keys from gen_batch_output, but only pop raw_prompt_ids and raw_response_ids; copy multi_modal_data
            # anchor_info = gen_batch_output.pop(
            #     non_tensor_batch_keys=anchor_keys,
            # )
            # anc_batch.non_tensor_batch.update(anchor_info.non_tensor_batch)

            # # Copy multi_modal_data instead of popping
            # anc_batch.non_tensor_batch["multi_modal_data"] = deepcopy(gen_batch_output.non_tensor_batch["multi_modal_data"])
            # anc_batch.meta_info = deepcopy(gen_batch_output.meta_info)
            # # Assign the batch from gen_batch_output for device placement
            # anc_batch.batch = gen_batch_output.batch
            ...
            # batch and meta_info are same as gen_batch_output for alignment
            #anc_batch_output.meta_info = gen_batch_output.meta_info
            new_batch = new_batch.union(gen_batch_output)
            #new_batch = new_batch.union(anc_batch_output)
