...
class FSDPWorker(Worker):
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def generate_anchors(self, prompts: DataProto):
        assert self._has_rollout
        ...
        meta_info = {
            ...
            "n": 1,  # no sampling when anchoring
            "temperature": 0.0,  # no sampling when anchoring
            "max_tokens": 1,
            "allowed_token_ids": [9693, 9454, 2514, 1866, 16, 15, 2152, 2753, 3849, 4049] # yes/Yes/True/true/1 or no/No/False/false/0
        }
        prompts.meta_info.update(meta_info)

        prompts = self.rollout_sharding_manager.preprocess_data(prompts)
        output = self.rollout.generate_anchors(prompts=prompts)
        output = self.rollout_sharding_manager.postprocess_data(output)
